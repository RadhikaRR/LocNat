...
public class DB_Branches {
	...
	public void createTable() {
		try {
			Statement st = database.createStatement("CREATE TABLE 'Branches' ( " + "'Name' TEXT, " + "'id' INTEGER )");
			st.prepare();
			st.execute();
		} catch (Exception e) {
			System.out.println(e.getMessage());
	...
	public void insertDB(BranchData data) {
		try {
			Statement st = database.createStatement("INSERT INTO Branches(Name,id) " + "VALUES (?,?)");
			st.prepare();
