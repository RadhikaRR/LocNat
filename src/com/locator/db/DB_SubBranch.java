...
public class DB_SubBranch {
	...
	public void createTable() {
		try {
			Statement st = database.createStatement("CREATE TABLE 'sub_branches' ( " + "'Name' TEXT, " + "'id' INTEGER )");
			st.prepare();
			st.execute();
		} catch (Exception e) {
			System.out.println(e.getMessage());
	...
	public void insertDB(SubBranchData data) {
		try {
			Statement st = database.createStatement("INSERT INTO sub_branches (Name,id) " + "VALUES (?,?)");
			st.prepare();
